...
class PostgresDBClient:
    ...
    def __init__(
        ...
        username = (
            ...
            else config.get("postgres", "username").strip()
        )
        password = (
            password
            if password is not None
            else config.get("postgres", "password").strip()
        )
        url = urlparse(
            url if url is not None else config.get("postgres", "url").strip()
        )
        database_name = (
            database_name
            if database_name is not None
            else config.get("postgres", "database_name").strip()
        )
