...
class ArangoDBClient:
    def __init__(
        ...
        self.client = ArangoClient(
            hosts=url if url is not None else config.get("arangodb", "url")
        )

        # Reads information about the graph
        self.db_name = (
            db_name if db_name is not None else config.get("arangodb", "db_name")
        )
        self.graph_name = (
            graph_name
            if graph_name is not None
            else config.get("arangodb", "graph_name")
        )

        # Rads information about the user. The password might be requested, if not present in config
        username = (
            username if username is not None else config.get("arangodb", "username")
        )
        password = (
            password
            if password is not None
                ...
                pw
                if (pw := config.get("arangodb", "password")) != "None"
                else getpass(
                    prompt=f"{client_name} requests ArangoDB password ({username}): "
                )
            )
        )
        ...
        try:
            ...
            try:
                ...
                if sys_db.has_database(self.db_name):
                    raise ValueError(
                        f"Arango user '{username}' has insufficient rights to connect to database '{self.db_name}'!"
                    )
                sys_db.create_database(self.db_name)
                logger.info(f"Created new ArangoDB database '{self.db_name}'.")
                self.db = self.client.db(
                    self.db_name, username=username, password=password
                )
            except:
                raise ValueError(
                    f"Arango user '{username}' can neither connect to nor create database '{self.db_name}'!"
                )
        ...
        if self.graph_name != "None":
            ...
            try:
                ...
                try:
                    ...
                    self.graph = self.db.graph(self.graph_name)
                    logger.info(
                        f"Created new graph '{self.graph_name}' in ArangoDB database '{self.db_name}'."
                    )
                except GraphListError:
                    raise ValueError(
                        f"Arango user {username} can not connect to graph {self.graph_name} in database {self.db_name}!"
                    )
        else:
            self.graph = None
    ...
    def get_node_description(self, node, condition=None) -> str:
        ...
        for edge_definition in self.graph.edge_definitions():
            ...
            outboundNodes = list(
                self.db.aql.execute(
                    f"FOR v, e in 1..1 outbound '{node['_id']}' graph '{self.graph_name}' OPTIONS {{'edgeCollections':'{edge_definition['edge_collection']}'}} return [v[\"label\"], e[\"label\"]]"
                )
            )
            relations = {}
            ...
            for edge in relations.keys():
                ...
                for nodeChunk in (
                    relations[edge][n : n + 5]
                    for n in range(0, len(relations[edge]), 5)
                ):
                    desc += (
                        f"{node['label']} has relation '{edge}' with "
                        + ", ".join(f"{elem}" for elem in nodeChunk)
                        + ". "
                    )
            ...
            inboundNodes = list(
                self.db.aql.execute(
                    f"FOR v, e in 1..1 inbound '{node['_id']}' graph '{self.graph_name}' OPTIONS {{'edgeCollections':'{edge_definition['edge_collection']}'}} return [v[\"label\"], e[\"label\"]]"
                )
            )
            relations = {}
            ...
            for edge in relations.keys():
                ...
                for nodeChunk in (
                    ...
                    desc += (
                        ", ".join(f"{elem}" for elem in nodeChunk)
                        + f" have relation '{edge}' with {node['label']}. "
                    )
    ...
    def get_vertex_count(self) -> int:
        ...
        return sum(
            self.db.aql.execute(
                f"return length(for v in {collection} return v)"
            ).__next__()
    ...
    def read_all_vertices(self):
        ...
        return (
            node
            for collection in self.graph.vertex_collections()
            for node in self.db.collection(collection)
        )
    ...
    def get_edge_count(self):
        ...
        return sum(
            self.db.aql.execute(
                f"return length(for e in {collection['edge_collection']} return e)"
            ).__next__()
            for collection in self.graph.edge_definitions()
        )
    ...
    def read_all_edges(self):
        ...
        return (
            edge
            for collection in self.graph.edge_definitions()
            for edge in self.db.collection(collection["edge_collection"])
        )

    def create_edge_collection(
        self, collection_name: str, nodes_from: [str], nodes_to: [str]
    ):
        """
        ...
        """
        if collection_name in self.edge_collections or self.graph.has_edge_collection(
            collection_name
        ):
            ...
            self.edge_collections[collection_name] = self.graph.create_edge_definition(
                ...
                from_vertex_collections=nodes_from,
                to_vertex_collections=nodes_to,
            )
    ...
    def get_edge_collection(self, collection_name: str) -> EdgeCollection:
        ...
        if collection_name in self.edge_collections:
            ...
            return False
            raise ValueError(
                f"Edge Collection '{collection_name}' unknown. Please create it first with create_edge_collection"
            )
