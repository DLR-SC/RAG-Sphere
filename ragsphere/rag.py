...
class RAG(BaseRAG):
    ...
    graph_db: Optional[
        Union[str, ArangoDBClient]
    ]  # Graph database, for now only str input supported
    vector_db: Optional[
        Union[str, Elasticsearch]
    ]  # Vector database, for now only str input supported
    indexer: Optional[
        Union[str, BaseIndexer]
    ]  # Low level indexer class or indexer name
    retriever: Optional[
        Union[str, BaseRetriever]
    ]  # Low level retriever class or indexer name
    ...
    def __init__(
        ...
        if isinstance(indexer_config, (dict, BaseIndexerConfig, type(None))):
            ...
            raise ValueError(
                "indexer_config must be of type 'dict' or 'BaseIndexerConfig'"
            )
        ...
        if isinstance(retriever_config, (dict, BaseRetrieverConfig, type(None))):
            ...
            raise ValueError(
                "retriever_config must be of type 'dict' or 'BaseRetrieverConfig'"
            )
        ...
        if indexer is None:
            ...
            try:
                ...
                raise ValueError(
                    f"Invalid indexer name: '{indexer}'. Supported: {[e.value for e in IndexerType]}"
                )
        ...
        if retriever is None:
            ...
            try:
                ...
                raise ValueError(
                    f"Invalid retriever name: '{retriever}'. Supported: {[e.value for e in RetrieverType]}"
                )
        ...
        if graph_db is None:
            ...
            try:
                ...
                raise ValueError(
                    f"Invalid graph_db: '{graph_db}'. Supported: {[e.value for e in DatabaseType]}"
                )
        else:
        ...
        if vector_db is None:
            ...
            try:
                ...
                raise ValueError(
                    f"Invalid vector_db: '{vector_db}'. Supported: {[e.value for e in DatabaseType]}"
                )
        else:
    ...
    def index(self, verbosity: Optional[int] = 1, **kwargs: Any) -> None:
        if verbosity:
            self._set_global_logging_level(verbosity)
        ...
        **kwargs: Any,
        ...
        if self.retriever is None:
            ...
            return self.query_engine.query(
                prompt, messages, retrieval_query=retrieval_query, **kwargs
            )

    def _get_llm(
        self, config: ConfigParser, index: bool = False, query: bool = False
    ) -> LLMClient:
    ...
    def test_llm_connection(self, llm_index: LLMClient, llm_query: LLMClient):
        ...
        try:
            ...
            logger.warning(
                "LLM Connection unavailable! Please check the parameters in the config file and llm server status!"
            )
    ...
    def _get_enum_type(
        ...
        if obj is None:
            ...
            raise TypeError(
                f"Expected {enum_class.__name__} or {base_class.__name__}, got {type(obj).__name__}"
            )
    ...
    def _supported_combinations_repr():
        ...
        for indexer, retriever in SUPPORTED_COMBINATIONS:
            ...
            lines.append(
                f"  - Indexer: {indexer_str:<20} Retriever: {retriever_str:<20}"
            )
        return "\n".join(lines)
