...
def _graph_rag_retrieve(
    ...
    confidence_cutoff = (
        40 if confidence_cutoff is None else max(0, min(100, confidence_cutoff))
    )
...
def _naive_graph_rag_retrieve(
    ...
    max_matches = 10 if max_matches is None else max(1, max_matches)
    confidence_cutoff = (
        0.04 if confidence_cutoff is None else max(0, min(1, confidence_cutoff))
    )
    ...
    communities = vector_db.search(
        index=vector_db_index_name, knn=query, source=["content", "source", "document"]
    )
    ...
    for community in sorted(
        ...
        results.append(
            RetrievalAnswer(
                **{
                    "name": list(
                        eval(community["_source"]["source"]).keys()
                    ).__repr__(),
                    "category": "extracted data from multiple different files (sources)",
                    "path": list(
                        eval(community["_source"]["document"]).keys()
                    ).__repr__(),
                    "type": AllowedTypes.TEXT,
...
def _garag_retrieve(
    ...
    valid_communities = vector_db.search(
        index=vector_db_index_name, knn=query, source=["source_ref"]
    )
