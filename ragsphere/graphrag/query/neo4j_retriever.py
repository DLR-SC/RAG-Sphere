...
def _graphrag_retrieve(
    ...
    retrieval_query: str = "MATCH (node)-[:AUTHORED_BY]->(author:Author)"
    "RETURN author.name",
    **kwargs: Any,
) -> Any:
    # DB Connection
    ...
    if (
        config.name == RetrieverType.VECTORGR or config.name == RetrieverType.VECTORGR
    ):  # cypher
    ...
    match config.name:
        case RetrieverType.VECTORGR:
            ...
            retriever_config = {"top_k": config.top_k, "filters": config.filters}
        ...
        case RetrieverType.HYBRIDCYPHERGR:  # Add result formatter
            ...
            retriever_config = {"top_k": config.top_k}  # Add query_params
    ...
    # Query the graph
    response = rag.search(
        query_text=prompt, retriever_config=retriever_config, return_context=True
    )
...
def _remove_searchable(structured_schema, exclude_label="Searchable"):
    ...
    cleaned["node_props"] = {
        k: v
        for k, v in structured_schema["node_props"].items()
    ...
    cleaned["rel_props"] = {
        k: v
        for k, v in structured_schema["rel_props"].items()
        if exclude_label not in k
    }
...
def _text2cypher_retrieve(
    ...
    **kwargs: Any,
    ...
    structured_schema = get_structured_schema(
        driver=driver, is_enhanced=False, database="neo4j", timeout=None, sanitize=False
    )
    ...
    formatted_schema = format_schema(schema=cleaned_schema, is_enhanced=False)
