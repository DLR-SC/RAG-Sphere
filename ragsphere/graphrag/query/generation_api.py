...
from ragsphere.graphrag.prompts import generate_community_answer
...
class GenerationAPI:
    ...
    def __init__(
        ...
        parallel_limit = (
            int(i)
            if (i := self.config_parser.get("general", "parallel_limit").strip()) != ""
            else 1
        )
        ...
        for i in range(5):
            try:
                ...
                elastic_client.options(
                    request_timeout=30, retry_on_status=[104]
                ).cluster.health()
                break
                ...
                if i < 4:
                    continue
                logger.error(
                    f"! Elasticsearch Connection unavailable! Please check the parameters in the config file and Elasticsearch server status!\n{format_exc()}"
                )
    ...
    def generate_graph_rag_answer(
        ...
        for degree in range(community_degree + 1):
            communities.extend(
                self.arango.get_aql(
                    f"FOR v IN CommunityNode FILTER v.community_degree == {degree} && v.is_copy == false && v.is_leaf == false RETURN v"
                )
            )
        ...
        while communities:
            ...
            if current_len >= 4096:
                ...
                informations.append(
                    (current_information, current_source, current_document)
                )
        ...
        pbar = (
            tqdm(total=len(informations), desc="Generating llm information")
            if show_progress
            else None
        )

        def generate_community_information(
            information, source, document
        ) -> Tuple[int, str, Set[str], Set[str]]:
            """
            ...
            while True:
                ...
                ai_info = self.llm.generate(
                    prompt=generate_community_answer.USER_PROMPT.format(
                        information=information, prompt=prompt
                    ),
                ...
                try:
                    ai_info = eval(ai_info)
                # Retry on failure
                except:
                    continue

                # Check for the object to be a dictionary
                if type(ai_info) != dict:
                    continue
                # Check for the needed keys in the dictionary
                if not "confidence" in ai_info or not "information" in ai_info:
                    continue
                # Check for the needed types in the dictionary
                if not type(ai_info["information"]) == str:
                    continue
                if not type(ai_info["confidence"]) == int:
                    continue
                # Response is valid
                break
        ...
        for information, source, document in informations:
            ...
            if show_progress:
                futures.append(
                    self.threadPool.submit(
                        lambda args: (
                            generate_community_information(*args),
                            pbar.update(),
                        )[0],
                        [information, source, document],
                    )
            ...
            # Show progress is disables -> generate information silently
                ...
                futures.append(
                    self.threadPool.submit(
                        generate_community_information, information, source, document
                    )
                )
        ...
        for confidence, information, source, document in sorted(
            informations, key=lambda i: i[0], reverse=True
        ):
    ...
    def generate_graph_rag_rag_answer(
        self, prompt: str, max_matches: int, confidence_cutoff: float = 0.04
    ) -> List[Dict]:
        """
        ...
        communities = self.elastic_client.search(
            index=self.GARAG_index_name,
            knn=query,
            source=["content", "source", "document"],
        )

        # Add all information until not important enough
        results = []
        for community in sorted(
            communities["hits"]["hits"], key=lambda c: c["_score"], reverse=True
        ):
            # The provided information will be more confusing than helpful. Break the loop
            if community["_score"] < confidence_cutoff:
                break
            # Information is added to the list of results
            results.append(
                {
    ...
    def generate_garag_answer(
        ...
        valid_communities = self.elastic_client.search(
            index=self.GARAG_index_name, knn=query, source=["source_ref"]
        )
        ...
        for community in valid_communities["hits"]["hits"]:
            ...
            for source, count in source_ref.items():
                if source == "_total":
                    continue
                source_refs[source] = (
                    source_refs.get(source, 0)
                    + community["_score"] * count / source_ref["_total"]
                )

        # Add at most max_matches initial document contents to the result using the source reference pointers to them
        results = []
        for source_ref, _ in sorted(
            source_refs.items(), key=lambda s: s[1], reverse=True
        ):
            # Fetch node described by source reference
            file_node = self.arango.get_aql(
                f"FOR v IN File FILTER v._key == '{source_ref}' LIMIT 1 RETURN v"
            ).__next__()
