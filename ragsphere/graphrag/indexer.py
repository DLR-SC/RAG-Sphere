...
from ragsphere.models.enums import IndexerType, DatabaseType
...
from ragsphere.graphrag.index.KG_1_LoadData import load_data
from ragsphere.graphrag.index.KG_2_ConvertTextsToGraph import generate_knowledge_graph
from ragsphere.graphrag.index.KG_3_ProcessKnowledgeGraph import process_knowledge_graph
from ragsphere.graphrag.index.KG_4_InitLeidenCommunities import build_communities
from ragsphere.graphrag.index.KG_5_CreateCommunitySummaries import summarize_communities
from ragsphere.graphrag.index.KG_6_CreateCommunityIndices import (
    generate_community_indices,
)
from ragsphere.graphrag.index.neo4j_indexer import _graphrag_index
...
class VectorGRIndexer(BaseIndexer):
    ...
    def index(self, **kwargs: Any) -> None:
        ...
        _graphrag_index(config=self.config, config_parser=self.config_parser)
...
def _index(
    ...
    if graph_db == DatabaseType.ARANGODB and config.name != IndexerType.VECTOR:
        ...
        db_name = "SC_Pub"
        arango_knowledge_graph_client = ArangoDBClient(
            config_parser, db_name=db_name, graph_name="knowledge_graph"
        )
        arango_community_graph_client = ArangoDBClient(
            config_parser, db_name=db_name, graph_name="community_graph"
        )

    ########################
    logger.info(
        f"({counter_inc()}/{total_method_calls}) Loading files into ArangoDB / ElasticSearch...\n"
    )
    try_method(
        method=load_data,
    ...
    logger.info(
        f"({counter_inc()}/{total_method_calls}) Generating knowledge graph...\n"
    )
    try_method(
        method=generate_knowledge_graph,
    ...
    logger.info(
        f"({counter_inc()}/{total_method_calls}) Processing knowledge graph...\n"
    )
    try_method(
        method=process_knowledge_graph,
    ...
    logger.info(
        f"({counter_inc()}/{total_method_calls}) Generating a community graph...\n"
    )
    try_method(
    ...
    logger.info(
        f"({counter_inc()}/{total_method_calls}) Summarizing community information...\n"
    )
    try_method(
        method=summarize_communities,
    ...
    logger.info(
        f"({counter_inc()}/{total_method_calls}) Creating elastic search indices for community information...\n"
    )
    try_method(
        method=generate_community_indices,
