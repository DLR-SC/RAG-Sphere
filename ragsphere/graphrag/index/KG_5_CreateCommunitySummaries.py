...
from ragsphere.graphrag.prompts import create_community_summary
...
def summarize_communities(
    ...
    parallel_limit = (
        int(i)
        if (i := config_parser.get("general", "parallel_limit").strip()) != ""
        else 0
    )
    ...
    vertices = list(
        community_graph.get_aql(
            f"FOR v IN 0..999999999 OUTBOUND '{graph_vertex['_id']}' GRAPH 'community_graph' RETURN v"
        )
    )
    ...
    with tqdm(total=len(vertices), desc="Summarizing communities") as pbar:
        with ThreadPoolExecutor(max_workers=1) as executor:
            ...
            def sum_vertex(vertex: dict):
                ...
                sub_vertices = list(
                    community_graph.get_aql(
                        f"FOR v IN OUTBOUND '{vertex['_id']}' GRAPH 'community_graph' RETURN v"
                    )
                )
                ...
                if len(sub_vertices) == 1:
                    ...
                    fields = {"_key": sub_vertices[0]["_key"], "is_copy": True}
                    community_graph.update_vertex("CommunityNode", fields)
                    return

                # Fetch the contents of the child vertices
                nodes = [
                    (*get_content(v), v["weight"], v["_id"], v["is_leaf"])
                ...
                if current_len >= max_char_count:
                    ...
                    while any(weights):
                        ...
                        if nodes[node_id][4]:
                            ...
                            sub_vertices = list(
                                community_graph.get_aql(
                                    f"FOR v IN OUTBOUND '{nodes[node_id][3]}' GRAPH 'community_graph' RETURN v"
                                )
                            )
                            ...
                            if (
                                current_len - nodes[node_id][0] + node_len
                                < max_char_count
                            ):
                ...
                while True:
                    # Generate a response
                    ai_summary = llm_client.generate(
                        prompt=create_community_summary.USER_PROMPT.format(
                            information=summary
                        ),
                    ...
                    # Try to read the data anyway
                        ...
                        if not '"description":' in ai_summary:
                            continue
                        ai_summary = ai_summary.split('"description":', maxsplit=1)
                        if "}" in ai_summary[0] or "{" in ai_summary[1]:
                            continue

                        if not '"label":' in ai_summary[0]:
                            continue
                        label = (
                            ai_summary[0]
                            .split('"label":', maxsplit=1)[1]
                            .strip(" ,\"'\n")
                        )
                        description = ai_summary[1].strip(" \"'}\n")

                        ai_summary = {"label": label, "description": description}

                        break

                    if not "label" in ai_summary or not "description" in ai_summary:
                        continue
                    if (
                        len(ai_summary["label"]) < 5
                        or len(ai_summary["description"]) < 20
                    ):
                        continue
                    break
