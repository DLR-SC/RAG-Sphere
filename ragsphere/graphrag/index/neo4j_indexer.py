...
def _graphrag_index(config: BaseIndexerConfig, config_parser: ConfigParser) -> None:
    # DB Connection
    ...
    if config.name == IndexerType.HYBRIDGR:
        ...
        for node in target_nodes:
            for prop in structured_schema["node_props"][node]:
                if (
                    counter > PROPERTY_LIMIT
                ):  # This is the limit before we get the max boolean clause error
                    break
