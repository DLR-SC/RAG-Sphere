...
def generate_community_indices(
    ...
    communities = list(
        community_graph.get_aql(
            f"FOR v IN CommunityNode FILTER v.is_copy == false && v.is_leaf == false RETURN v"
        )
    )
    # Embed all community contents into vectors
    embeddings = transformer.encode(
        [community["content"] for community in communities], show_progress_bar=True
    )
    # Add each community with its information and content vector into elasticsearch
    for community, content_vector in tqdm(
        list(zip(communities, embeddings)), desc="Indexing communities"
    ):
        embedded = {
