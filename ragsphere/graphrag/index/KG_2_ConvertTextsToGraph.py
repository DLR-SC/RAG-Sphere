...
from ragsphere.graphrag.prompts import convert_to_graph
from ragsphere.utils.arango_client import ArangoDBClient
from ragsphere.graphrag.index.KG_convert_to_relations import _try_get_relations
...
def _insert_relations(
    ...
    for relation in relations:
        ...
        if not type(relation) == dict:
            continue
        # If a value is missing, skip this relation
        if not "From" in relation or not "To" in relation or not "Relation" in relation:
            continue
        ...
        relation["To"] = sub(
            r'[^A-Za-z0-9_\-.@()+=;$!*%:,}{\]["]+', "", relation["To"].replace(" ", "_")
        )
        ...
        if not relation["From"] or not relation["To"] or not relation["Relation"]:
            continue
        ...
        try:
            with _LOCK:
                ...
                from_node = (
                    list(
                        arango_client.get_aql(
                            "FOR v IN Node FILTER v._key == '{}' LIMIT 1 RETURN v".format(
                                relation["From"]
                            )
                        )
                    )
                ...
                if from_node == None:
                    ...
                    fields = {
                        ...
                        "source_ref": _update_source_dict(
                            from_node["source_ref"], source_ref
                        ),
                        "document": _update_source_dict(
                            from_node["document"], document
                        ),
                    }
                ...
                edge = list(
                    arango_client.get_aql(
                        "FOR e IN Relation FILTER e._from == '{}' && e._to == '{}' && e.label == '{}' LIMIT 1 RETURN e".format(
                            from_node["_id"], to_node["_id"], relation["Relation"]
                        )
                    )
                )
                if not edge:
                    # Add new edge
                    arango_client.add_edge(
                        "Relation",
                        {
                            "_from": from_node["_id"],
                            "_to": to_node["_id"],
                    ...
                    arango_client.update_edge(
                        "Relation",
                        {"_key": edge[0]["_key"], "weight": edge[0]["weight"] + 1},
                    )
        except:
            print(relation)
            print(format_exc())
            ...
            for file in files:
                ...
                def generate_ner_results(content, file_key, source, document):
                    ...
                    for _ in range(8):
                        ...
                        llm_result = ner_re_llm.generate(
                            prompt=convert_to_graph.USER_PROMPT.format(
                                information=content
                            ),
                        ...
                        if (
                            type(relations) == list
                            and any(relations)
                            and type(relations[0]) == dict
                        ):
                            break
                    else:
                        # Could not generate valid response in 8 tries. Skip this content entirely
                        with _LOCK:
                            arango_client.update_vertex(
                                "File", {"_key": file_key, "is_graph": True}
                            )
                        return
                ...
                executor.submit(
                    lambda args: (
                        generate_ner_results(*args),
                        pbar.update(),
                        pbar.refresh(),
                    ),
                    [file["content"], file["_key"], file["source"], file["document"]],
                )
