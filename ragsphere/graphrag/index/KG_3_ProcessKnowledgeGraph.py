...
def process_knowledge_graph(
    ...
    for node in tqdm(nodes, desc="Connecting nodes to sources"):
        ...
        connected_vertices = set(
            arango_client.get_aql(
                f"FOR v IN 1..3 ANY '{node['_id']}' GRAPH {arango_client.graph_name} \
        ...
        for source, count in node["source_ref"].items():
            ...
            if source in connected_vertices:
                continue
    ...
    for source, count in tqdm(
        source_refs.items(), desc="Calculate source reference weight modifiers"
    ):
    ...
    for node in tqdm(nodes, desc="Assigning node weights"):
        ...
        fields = {"_key": node["_key"], "weight": weight}
