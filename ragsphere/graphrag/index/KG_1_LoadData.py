...
from ragsphere.utils.file_parsers.docxParser import parse_docx
from ragsphere.utils.file_parsers.pdfParser import parse_pdf
from ragsphere.utils.file_parsers.txtParser import parse_txt
from ragsphere.utils.file_parsers.RAGSplit import split
from ragsphere.utils.file_parsers.zipExtractor import extracted_zip
...
def _load_file(
    ...
    if file_path.is_dir():
        ...
        if arango_client:
            if not arango_client.get_aql(
                "FOR v IN File FILTER v.file_path == '{}' LIMIT 1 RETURN v".format(
                    str(file_path.absolute()).replace("\\", "\\\\")
                )
            ).empty():
                # Skip file
                logger.info(
                    f"({file_path}) File already present in ArangoDB. Content gets skipped!"
                )
                return
        ...
        try:
            ...
            logger.error(
                f"({file_path}) An error ocurred whilst reading a file: {format_exc()}"
            )
        ...
        for text_chunk in data:
            ...
            if arango_client:
                with _LOCK:
                    ...
                    fields = {
                        ...
                        "document": {file_path.name: 1},
                        "source": {file_path.name + " " + text_chunk[b"PageHint"]: 1},
                        "label": (
                            file_path.name + " - " + text_chunk[b"h1"].replace("*", "")
                            if text_chunk[b"h1"]
                            else file_path.name
                        ),
                        "weight": 1,
                    ...
                    fields = {
                        "_key": node["_key"],
                        "source_ref": {node["_key"]: 1, "_total": 1},
                    }
            ...
            if elastic_tuple:
                ...
                embedding = embedding_model.encode(
                    text_chunk[b"Content"], show_progress_bar=False
                )
                ...
                with _LOCK:
                    ...
                    elastic_client.index(
                        index=index_name, id=uuid4(), document=embedded
                    )

    else:
        # File is of unsupported type
        logger.error(
            f"({file_path}) {file_path.suffix.lower()} type files are not yet supported!"
        )
...
def load_data(
    ...
    parallel_limit = int(
        config_parser.get("general", "parallel_limit").strip()
    )  # in dataclass config

    # Setup arangodb enviromnent
    if arango_client and config.name != IndexerType.VECTOR:
        arango_client.add_vertex_index(
            "File", {"type": "persistent", "fields": ["file_path"], "unique": False}
        )
    ...
    if elastic_tuple:
        ...
        elastic_client.options(ignore_status=[400]).indices.create(
            index=index_name, mappings=elastic_mappings
        )
    ...
    with tqdm(total=len(paths), desc="Loading data from files") as pbar:
        with ThreadPoolExecutor(max_workers=parallel_limit) as executor:
            for path in paths:
                executor.submit(
                    lambda args: (_load_file(*args), pbar.update()),
                    [config, path, arango_client, elastic_tuple, max_chunk_size],
                )
