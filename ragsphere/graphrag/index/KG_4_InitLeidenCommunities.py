...
from ragsphere.graphrag.index.G_LeidenAlgorithm import LeidenProcessor
...
def build_communities(
    ...
    for vertex_id, communities in tqdm(
        vertex_communities.items(), desc="Saving vertex communities"
    ):
        # Add a list of community ids, reflecting the communities on different levels
        fields = {"_id": vertex_id, "communities": communities}
        ...
        knowledge_graph.update_vertex(
            knowledge_graph.get_type_from_id(vertex_id), fields
        )
    ...
    try:
        for community in tqdm(
            list(community_graph.get_aql("FOR v IN CommunityNode RETURN v")),
            desc="Copying old graph data",
        ):
            if community["content"] == "_":
                continue
    ...
    community_graph.add_vertex_index(
        "CommunityNode",
        {"type": "persistent", "fields": ["community_key"], "unique": True},
    )
    community_graph.add_vertex_index(
        "CommunityNode",
        {"type": "persistent", "fields": ["community_degree"], "unique": False},
    )

    # Loop over every community vertex in the graph and add it to the ArangoDB
    for community, degree, index in tqdm(
        leiden_community_graph["vertices"], desc="Constructing community nodes"
    ):
        ...
        for vertex_id in community:
            ...
            vertex = knowledge_graph.get_aql(
                f"FOR v IN {knowledge_graph.get_type_from_id(vertex_id)} FILTER v._id == '{vertex_id}' LIMIT 1 RETURN v"
            ).__next__()
            ...
            if not vertex_id in node_edges:
                node_edges[vertex_id] = set(
                    knowledge_graph.get_aql(
                        f"FOR v,e IN 1..1 ANY '{vertex_id}' GRAPH knowledge_graph RETURN e._id"
                    )
                )
        ...
        community_keys[(community, degree)] = community_graph.add_vertex(
            "CommunityNode", fields
        )

    # Add an edge collection to connect the communities into an hierarchical structure
    community_graph.create_edge_collection(
        "communityEdge", ["CommunityNode"], ["CommunityNode"]
    ).truncate()

    # Add all edges between the communities
    for (from_vertex, to_vertex), edges in tqdm(
        leiden_community_graph["edges"].items(), desc="Constructing community edges"
    ):
    ...
    graph_vertex = community_graph.get_aql(
        f"FOR v IN CommunityNode FILTER v.community_key == '00000/00000' LIMIT 1 RETURN v"
    ).__next__()

    # Add a description for each leave node and insert the saved data
    for community in tqdm(
        list(community_graph.get_aql(f"FOR v IN CommunityNode RETURN v")),
        desc="Loading node descriptions",
    ):
        # Insert saved data
        if (
            save := data_saves.get(
                tuple(community["vertices"] + community["edges"]), None
            )
        ) != None:
            community_graph.update_vertex(
                "CommunityNode", save | {"_key": community["_key"]}
            )
        # Create new leave descriptions
            ...
            knowledge_node = knowledge_graph.get_aql(
                f"FOR v IN {knowledge_graph.get_type_from_id(vertex)} FILTER v._id == '{vertex}' LIMIT 1 RETURN v"
            ).__next__()
